		...
		r.Route("/rooms", func(r chi.Router){
			...
			r.Route("/{room_id}/messages", func(r chi.Router){
				...
				r.Route("/{message_id}", func(r chi.Router){
					...
					r.Patch("/answer", a.handleMarkMessageAsAswered)
				})
			})
		})
	})
...
func (h apiHandler) handleNotifyClient(msg Message) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
h.mu.Lock()
	...
	if err != nil {
		slog.Error("Failde to Insert Message", "error", err)
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	}
	...
	go h.handleNotifyClient(Message{
...
func (h apiHandler) handleMarkMessageAsAswered(w http.ResponseWriter, r *http.Request) {
	_, rawRoomID, _, ok := h.readRoom(w, r)
	if !ok {
		return
	}
